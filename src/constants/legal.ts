export const LEGAL_LAST_UPDATED =
  '20 Agustus 2026';

export const TERMS_CONTENT = `
SYARAT DAN KETENTUAN KALOO POS

Terakhir Diperbarui: ${LEGAL_LAST_UPDATED}

Selamat datang di KALOO POS, layanan Point of Sale (POS), pengelolaan usaha, pemesanan mandiri/kiosk, transaksi, pembayaran, voucher, member, loyalitas, inventaris, laporan, serta fitur pendukung lain yang dikelola oleh Evognito.

Dengan mendaftar, mengakses, atau menggunakan KALOO POS, Anda menyatakan telah membaca, memahami, dan menyetujui Syarat dan Ketentuan ini.

1. RUANG LINGKUP LAYANAN

KALOO POS menyediakan perangkat lunak untuk membantu kegiatan operasional usaha, termasuk tetapi tidak terbatas pada:
- Pengelolaan produk, kategori, harga, dan stok
- Transaksi kasir dan pencatatan pesanan
- Pemesanan melalui kiosk atau perangkat pelanggan
- Pengelolaan cabang, meja, staf, dan akun pengguna
- Pembayaran tunai, QRIS, dan metode pembayaran lain yang tersedia
- Voucher, promosi, member, poin, dan program loyalitas
- Laporan transaksi, analitik, dan administrasi usaha
- Integrasi API dan layanan pihak ketiga.

KALOO POS merupakan penyedia teknologi. Produk, layanan, harga, kualitas, ketersediaan, pemenuhan pesanan, dan hubungan jual beli dengan pelanggan tetap menjadi tanggung jawab mitra atau pemilik usaha.

2. KELAYAKAN DAN AKUN

Anda wajib:
- Memberikan data pendaftaran yang benar, lengkap, dan terkini
- Menggunakan akun sesuai kewenangan yang diberikan
- Menjaga kerahasiaan kata sandi, token, kode akses, dan kredensial lainnya
- Segera melaporkan akses tanpa izin atau aktivitas mencurigakan
- Memastikan staf yang menggunakan akun memahami dan mematuhi ketentuan ini.

Seluruh aktivitas melalui akun dianggap dilakukan oleh pemilik akun sampai kami menerima pemberitahuan mengenai penyalahgunaan akun.

3. KEWAJIBAN PENGGUNA DAN MITRA

Pengguna atau mitra bertanggung jawab untuk:
- Memiliki perizinan usaha yang diperlukan
- Memastikan informasi produk, harga, stok, pajak, diskon, dan promosi akurat
- Memenuhi kewajiban perlindungan konsumen, perpajakan, ketenagakerjaan, dan kewajiban hukum lainnya
- Memperoleh persetujuan atau dasar pemrosesan yang sah sebelum memasukkan data pelanggan dan staf
- Menjaga keamanan perangkat kasir, kiosk, printer, jaringan, dan akun staf
- Menangani keluhan pelanggan terkait produk, kualitas, pemenuhan pesanan, pembatalan, dan refund.

4. PENGGUNAAN YANG DILARANG

Anda dilarang:
- Menggunakan KALOO POS untuk kegiatan ilegal, penipuan, pencucian uang, atau perdagangan barang terlarang
- Mengakses akun, server, API, atau data tanpa izin
- Mengunggah malware, script berbahaya, atau melakukan serangan terhadap sistem
- Memalsukan transaksi, identitas, voucher, status pembayaran, atau data member
- Melakukan reverse engineering, menyalin, menjual kembali, atau mendistribusikan kode dan desain tanpa izin tertulis
- Menghindari batas paket, kontrol akses, atau mekanisme keamanan
- Menggunakan bot, scraper, atau otomasi yang membebani atau merugikan layanan.

5. TRANSAKSI DAN PEMBAYARAN

Pembayaran dapat diproses oleh bank, payment gateway, penyedia QRIS, atau penyedia layanan pembayaran lainnya.

Pengguna memahami bahwa:
- Keberhasilan pembayaran bergantung pada konfirmasi penyedia pembayaran
- Gangguan bank, jaringan, payment gateway, atau perangkat dapat memengaruhi status transaksi
- Refund, pembatalan, chargeback, dan rekonsiliasi mengikuti kebijakan mitra, penyedia pembayaran, dan hukum yang berlaku
- KALOO POS tidak menyimpan PIN, CVV, atau kredensial perbankan pelanggan
- Catatan transaksi elektronik, nomor pesanan, dan struk dapat digunakan sebagai bukti transaksi sesuai hukum yang berlaku.

6. LANGGANAN, BIAYA, DAN PENAGIHAN

Fitur tertentu dapat memerlukan langganan berbayar.

Dengan berlangganan, Anda menyetujui:
- Biaya, periode tagihan, batas penggunaan, dan fitur paket yang ditampilkan
- Pembayaran sesuai siklus tagihan
- Pembatasan atau penangguhan fitur apabila pembayaran tertunggak
- Perubahan harga yang diberitahukan sebelum periode tagihan berikutnya.

Biaya yang telah dibayarkan tidak dapat dikembalikan kecuali diwajibkan oleh hukum, terjadi kesalahan penagihan, atau dinyatakan lain secara tertulis.

7. VOUCHER, PROMO, MEMBER, DAN LOYALITAS

Voucher, promosi, dan manfaat member tunduk pada:
- Periode berlaku
- Batas penggunaan
- Minimum transaksi
- Cabang yang berlaku
- Status member
- Kuota dan ketentuan lain yang ditetapkan mitra.

Voucher tidak dapat diuangkan atau dipindahtangankan kecuali dinyatakan lain.

Poin loyalitas bukan uang elektronik dan tidak memiliki nilai tunai. Poin atau voucher dapat dibatalkan apabila diperoleh melalui kesalahan sistem, manipulasi, transaksi tidak sah, refund, atau penyalahgunaan.

8. KIOSK DAN PERANGKAT BERSAMA

Pengguna kiosk wajib memastikan pesanan, identitas pelanggan, voucher, dan metode pembayaran telah benar sebelum menyelesaikan transaksi.

Data member hanya boleh dimasukkan oleh pemilik data atau dengan izin pemiliknya.

Sesi kiosk dapat direset otomatis setelah tidak aktif untuk melindungi data dan mencegah penyalahgunaan.

Mitra bertanggung jawab menempatkan perangkat kiosk secara aman dan membatasi akses administratif.

9. API DAN INTEGRASI

Penggunaan API atau integrasi wajib mengikuti dokumentasi, batas penggunaan, dan mekanisme keamanan KALOO POS.

API key, token, secret, dan kredensial integrasi wajib dijaga kerahasiaannya.

KALOO POS dapat membatasi atau mencabut akses API apabila terjadi penyalahgunaan, beban berlebihan, risiko keamanan, atau pelanggaran ketentuan.

Ketersediaan integrasi pihak ketiga dapat berubah sesuai kebijakan pihak tersebut.

10. HAK KEKAYAAN INTELEKTUAL

Seluruh sistem, kode sumber, desain, logo, dokumentasi, merek, dan komponen KALOO POS merupakan milik Evognito atau pemberi lisensinya dan dilindungi oleh hukum kekayaan intelektual.

Mitra tetap memiliki data dan konten yang dimasukkan ke dalam sistem.

Mitra memberikan izin terbatas kepada KALOO POS untuk menyimpan, memproses, menampilkan, dan menggunakan konten tersebut sejauh diperlukan untuk menyediakan layanan.

11. KETERSEDIAAN DAN PEMELIHARAAN

KALOO POS berupaya menjaga layanan tersedia, stabil, dan aman, tetapi tidak menjamin layanan selalu bebas gangguan atau kesalahan.

Pemeliharaan, pembaruan, serangan siber, gangguan internet, listrik, pusat data, bank, payment gateway, atau keadaan lain dapat memengaruhi ketersediaan.

Mitra disarankan memiliki prosedur operasional cadangan, termasuk pencatatan manual dan ekspor laporan secara berkala.

12. PENANGGUHAN DAN PENGHENTIAN

KALOO POS dapat membatasi, menangguhkan, atau menghentikan akun apabila:
- Terjadi pelanggaran ketentuan
- Terdapat risiko keamanan
- Pembayaran tertunggak
- Penggunaan mengganggu sistem atau pengguna lain
- Akun digunakan untuk kegiatan ilegal atau penipuan
- Tindakan tersebut diwajibkan oleh hukum atau otoritas.

Pemberitahuan akan diberikan apabila memungkinkan, kecuali tindakan segera diperlukan untuk keamanan atau kepatuhan hukum.

13. PEMBATASAN TANGGUNG JAWAB

Dalam batas yang diizinkan hukum, KALOO POS tidak bertanggung jawab atas:
- Kerugian tidak langsung
- Kehilangan keuntungan, peluang, reputasi, atau data
- Kesalahan input, konfigurasi, harga, atau stok oleh pengguna
- Tindakan staf, pelanggan, penyedia pembayaran, atau pihak ketiga
- Gangguan akibat perangkat, internet, listrik, atau keadaan di luar kendali wajar.

Pembatasan ini tidak mengurangi hak konsumen yang wajib dilindungi berdasarkan hukum Indonesia.

14. KEADAAN KAHAR

KALOO POS tidak bertanggung jawab atas kegagalan layanan akibat kejadian di luar kendali wajar, termasuk bencana alam, kebakaran, banjir, perang, kerusuhan, epidemi, gangguan listrik, internet, telekomunikasi, pusat data, pemerintah, bank, atau penyedia pembayaran.

15. PERUBAHAN LAYANAN DAN KETENTUAN

KALOO POS dapat memperbarui fitur, harga, dan ketentuan untuk menyesuaikan perkembangan layanan, teknologi, keamanan, dan hukum.

Perubahan material akan diberitahukan melalui aplikasi, situs web, email, atau kanal lain yang wajar.

Penggunaan layanan setelah tanggal berlaku perubahan dianggap sebagai penerimaan terhadap ketentuan terbaru, sepanjang diperbolehkan oleh hukum.

16. HUKUM DAN PENYELESAIAN SENGKETA

Ketentuan ini tunduk pada hukum Republik Indonesia.

Sengketa akan terlebih dahulu diselesaikan melalui musyawarah. Apabila tidak tercapai penyelesaian, sengketa dapat diajukan ke forum yang berwenang sesuai hukum Indonesia tanpa mengurangi hak konsumen.

17. KONTAK

Untuk pertanyaan mengenai Syarat dan Ketentuan, hubungi:

KALOO POS / Evognito
Email: support@kaloopos.com
Wilayah Operasional: Indonesia
`;

export const PRIVACY_CONTENT = `
KEBIJAKAN PRIVASI KALOO POS

Terakhir Diperbarui: ${LEGAL_LAST_UPDATED}

KALOO POS berkomitmen melindungi data pribadi pengguna, mitra, staf, member, dan pelanggan yang menggunakan layanan Point of Sale, dashboard, kiosk, pembayaran, voucher, loyalitas, API, dan fitur terkait lainnya.

Kebijakan ini menjelaskan cara KALOO POS memperoleh, menggunakan, menyimpan, membagikan, melindungi, serta menghapus data pribadi.

1. PERAN KALOO POS DAN MITRA

Dalam pengelolaan akun KALOO POS, langganan, keamanan sistem, dukungan, dan pengembangan layanan, KALOO POS dapat bertindak sebagai Pengendali Data Pribadi.

Untuk data pelanggan dan staf yang dimasukkan oleh mitra, mitra pada umumnya bertindak sebagai Pengendali Data Pribadi dan KALOO POS bertindak sebagai Prosesor Data Pribadi yang memproses data berdasarkan instruksi mitra.

Mitra bertanggung jawab memastikan data pelanggan dan staf diperoleh dan digunakan secara sah.

2. DATA YANG DIKUMPULKAN

KALOO POS dapat memproses:

A. Data identitas dan akun
- Nama
- Email
- Nomor telepon
- ID member
- Username
- Peran pengguna
- Nama dan informasi usaha.

B. Data autentikasi dan keamanan
- Kata sandi yang telah di-hash
- Token sesi
- Kode verifikasi
- Catatan login
- Alamat IP
- Aktivitas keamanan akun.

C. Data usaha dan operasional
- Cabang
- Produk
- Kategori
- Harga
- Stok
- Meja
- Staf
- Jadwal
- Pengaturan outlet.

D. Data transaksi
- Nomor pesanan
- Isi keranjan
- Add-on
- Catatan pesanan
- Jumlah transaksi
- Diskon
- Voucher
- Pajak
- Metode dan status pembayaran
- Waktu dan riwayat transaksi.

E. Data member dan loyalitas
- Nama
- Email
- Nomor telepon
- ID member
- Poin
- Riwayat transaksi
- Voucher dan manfaat yang digunakan.

F. Data pembayaran
- Metode pembayaran
- Status pembayaran
- Nomor referensi transaksi
- Informasi QRIS
- Informasi yang diberikan penyedia pembayaran.

KALOO POS TIDAK menyimpan PIN, CVV, atau kata sandi perbankan pelanggan.

G. Data perangkat dan teknis
- Jenis perangkat
- Browser
- Sistem operasi
- Alamat IP
- Log server
- Waktu akses
- Data diagnostik
- Cookie dan penyimpanan lokal.

H. Data komunikasi
- Pesan dukungan
- Laporan masalah
- Umpan balik
- Korespondensi dengan tim KALOO POS.

3. SUMBER DATA

Data dapat diperoleh:
- Langsung dari pengguna
- Dari mitra atau organisasi tempat pengguna bekerja
- Dari pelanggan saat melakukan transaksi
- Secara otomatis dari perangkat dan penggunaan layanan
- Dari payment gateway, bank, penyedia QRIS, atau integrasi lain
- Dari sumber lain yang sah sesuai hukum.

4. TUJUAN PENGGUNAAN DATA

Data digunakan untuk:
- Menyediakan dan mengoperasikan layanan
- Membuat dan mengelola akun
- Memproses pesanan, pembayaran, voucher, dan poin
- Mengelola POS, kiosk, stok, cabang, dan laporan
- Memverifikasi identitas dan member
- Mencegah penipuan dan akses tanpa izin
- Mengirim notifikasi transaksi, keamanan, tagihan, dan layanan
- Memberikan dukungan pelanggan
- Melakukan analisis, pengujian, dan pengembangan fitur
- Memenuhi kewajiban hukum, akuntansi, perpajakan, dan audit
- Menyelesaikan sengketa dan melindungi hak pengguna serta KALOO POS.

5. DASAR PEMROSESAN

Pemrosesan dilakukan berdasarkan satu atau lebih dasar berikut:
- Persetujuan
- Pelaksanaan perjanjian
- Kewajiban hukum
- Kepentingan vital
- Kepentingan umum
- Kepentingan sah KALOO POS atau pihak lain yang tidak mengesampingkan hak pengguna.

Apabila pemrosesan didasarkan pada persetujuan, pengguna dapat menarik persetujuan sesuai mekanisme yang tersedia.

6. DATA PELANGGAN MILIK MITRA

Mitra bertanggung jawab:
- Memberikan pemberitahuan privasi kepada pelanggan
- Memperoleh persetujuan atau dasar pemrosesan yang sah
- Memastikan data yang dimasukkan relevan dan akurat
- Membatasi akses staf sesuai kebutuhan
- Menangani permintaan pelanggan terkait data mereka.

KALOO POS akan membantu mitra memenuhi kewajiban perlindungan data sepanjang memungkinkan dan sesuai layanan yang digunakan.

7. PEMBAGIAN DATA

Data dapat dibagikan secara terbatas kepada:

A. Penyedia infrastruktur
Penyedia hosting, cloud, database, penyimpanan, analitik, email, notifikasi, dukungan, dan keamanan.

B. Penyedia pembayaran
Bank, payment gateway, penyedia QRIS, dan pihak terkait pemrosesan pembayaran.

C. Mitra dan administrator
Pemilik usaha, administrator, cabang, atau staf yang memiliki kewenangan.

D. Otoritas
Instansi pemerintah, pengadilan, aparat penegak hukum, atau regulator apabila diwajibkan oleh hukum.

E. Transaksi korporasi
Pihak yang terlibat dalam merger, akuisisi, restrukturisasi, pembiayaan, atau pengalihan aset dengan perlindungan kerahasiaan yang sesuai.

KALOO POS tidak menjual data pribadi kepada pengiklan.

8. TRANSFER DATA LINTAS NEGARA

Sebagian penyedia teknologi dapat memproses data di luar Indonesia.

Apabila terjadi transfer lintas negara, KALOO POS akan menerapkan dasar dan perlindungan yang diwajibkan oleh hukum, termasuk penilaian tingkat perlindungan, kontrak, persetujuan, atau mekanisme lain yang sah.

9. PENYIMPANAN DATA

Data disimpan selama diperlukan untuk:
- Menyediakan layanan
- Menjalankan kontrak
- Memenuhi kewajiban hukum
- Menjaga keamanan
- Menyelesaikan sengketa
- Memenuhi kebutuhan akuntansi dan perpajakan.

Setelah masa penyimpanan berakhir, data dapat dihapus, dimusnahkan, atau dianonimkan, kecuali penyimpanan lebih lama diwajibkan oleh hukum.

10. KEAMANAN DATA

KALOO POS menerapkan langkah teknis dan organisasi yang wajar, termasuk:
- Kontrol akses
- Autentikasi
- Hashing kata sandi
- Enkripsi saat transmisi
- Pencatatan aktivitas
- Pencadangan
- Pembatasan akses staf
- Pemantauan dan pembaruan keamanan.

Tidak ada sistem elektronik yang sepenuhnya bebas risiko. Pengguna wajib menjaga kredensial dan keamanan perangkat.

11. INSIDEN KEAMANAN

Apabila terjadi kegagalan perlindungan data pribadi, KALOO POS akan melakukan penanganan, mitigasi, dokumentasi, dan pemberitahuan kepada pihak yang diwajibkan sesuai peraturan yang berlaku.

12. HAK SUBJEK DATA

Sesuai hukum yang berlaku, pengguna dapat memiliki hak untuk:
- Memperoleh informasi mengenai pemrosesan data
- Mengakses data pribadi
- Memperoleh salinan data
- Memperbarui atau memperbaiki data
- Menarik persetujuan
- Meminta penghentian, pembatasan, penghapusan, atau pemusnahan data
- Mengajukan keberatan terhadap keputusan otomatis
- Memperoleh atau memindahkan data dalam format yang lazim apabila memungkinkan
- Mengajukan pengaduan dan meminta ganti rugi sesuai hukum.

Permintaan dapat memerlukan verifikasi identitas.

Apabila data dikendalikan oleh mitra, KALOO POS dapat mengarahkan permintaan kepada mitra terkait.

13. COOKIE DAN PENYIMPANAN LOKAL

KALOO POS dapat menggunakan cookie atau penyimpanan lokal untuk:
- Login dan sesi
- Keamanan
- Keranjang belanja
- Preferensi pengguna
- Analitik dan peningkatan layanan.

Menonaktifkan cookie tertentu dapat menyebabkan sebagian fungsi tidak berjalan.

14. DATA ANAK

Layanan KALOO POS ditujukan untuk kegiatan usaha dan transaksi umum.

Anak yang belum cakap secara hukum harus menggunakan layanan dengan persetujuan atau pendampingan orang tua atau wali apabila diwajibkan oleh hukum.

15. TAUTAN DAN PIHAK KETIGA

Layanan dapat terhubung dengan situs, aplikasi, payment gateway, atau layanan pihak ketiga.

Penggunaan layanan pihak ketiga tunduk pada kebijakan privasi dan ketentuan mereka sendiri.

16. PENGHAPUSAN AKUN DAN DATA

Pengguna dapat meminta penutupan akun atau penghapusan data dengan menghubungi tim dukungan.

Penghapusan dapat dibatasi apabila data masih diperlukan untuk:
- Kewajiban hukum
- Transaksi dan pembukuan
- Perpajakan
- Pencegahan penipuan
- Penyelesaian sengketa
- Kepentingan hukum lain yang sah.

17. PERUBAHAN KEBIJAKAN

KALOO POS dapat memperbarui Kebijakan Privasi ini untuk menyesuaikan perubahan hukum, teknologi, dan layanan.

Perubahan material akan diberitahukan melalui aplikasi, situs, email, atau kanal lain yang wajar.

18. KONTAK DAN PENGADUAN

Untuk pertanyaan, permintaan hak data, atau pengaduan privasi, hubungi:

KALOO POS / Evognito
Email: support@kaloopos.com
Wilayah Operasional: Indonesia
`;