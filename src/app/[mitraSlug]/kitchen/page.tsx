...
export default function KitchenDisplay({ onLogout: parentLogout }: Props) {
  ...
  useEffect(() => {
    ...
    const verifySession = async () => {
      try {
        // Panggil API yang mengecek Cookie ekasir_session
        const res = await fetch(`/api/auth/me?slug=${slug}`);
        const data = await res.json();
        
        if (data.success && data.user) {
          setActiveStaffName(data.user.name);
          setIsAuthenticated(true);
        } else {
          // Jika server bilang tidak ada session/token kadaluarsa
          router.push('/login');
        }
      } catch (e) {
        router.push('/login');
  ...
  const executeUpdate = async (orderId: string, newStatus: Order['status']) => {
    ...
    try {
      const response = await fetch('/api/pos/kitchen/orders', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      console.error('Gagal memperbarui status kitchen:', error);
      fetchOrders(true);
    }
  };
