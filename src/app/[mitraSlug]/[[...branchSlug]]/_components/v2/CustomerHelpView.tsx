...
export default function SupportView() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f7f7f5]">
        <Icons.Loader2 className="w-10 h-10 text-[#111111] animate-spin mb-4" />
  ...
  return (
    <div className="min-h-full bg-stone-50 px-4 pb-32 pt-6">
      {/* Toast Notification */}
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            className="fixed left-1/2 top-6 -translate-x-1/2 z-[100] bg-stone-900/90 backdrop-blur-md text-white px-4 py-3.5 rounded-2xl shadow-2xl border border-stone-700/50 flex items-center gap-3.5 w-[90%] max-w-sm overflow-hidden"
          >
            {/* Garis Aksen Hijau di kiri agar lebih elegan */}
            <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-[#111111]" />
      ...
      <header className="mb-8">
        ...
        <h1 className="mb-3 text-[34px] font-black leading-[0.98] tracking-[-0.055em] text-neutral-950">
          Bantuan &<br /> Layanan
        </h1>
      ...
      {hasWifiInfo && (
        <div className="mb-10 grid grid-cols-1 gap-3">
           <motion.div 
             whileHover={{ y: -4 }}
             className="rounded-[26px] border border-black/[0.07] bg-white p-5 text-left"
           >
             <div className="w-14 h-14 bg-emerald-50 text-[#111111] rounded-full flex items-center justify-center mb-6">
             ...
             <button
               onClick={handleWifi}
               className="w-full py-3 bg-stone-50 text-stone-700 border border-stone-100 rounded-xl text-xs font-bold uppercase hover:bg-[#111111] hover:text-white transition-all active:scale-95 shadow-sm"
             >
               Lihat Sandi
             </button>
           </motion.div>
        ...
        <section>
           ...
           <div className="space-y-4">
             {settings.faqs!.map((faq, index) => (
               <div key={index} className="rounded-2xl border border-black/[0.07] bg-white p-5">
                 <div className="flex items-center gap-3 mb-3">
                   <Icons.HelpCircle className="w-4 h-4 text-[#111111]" />
