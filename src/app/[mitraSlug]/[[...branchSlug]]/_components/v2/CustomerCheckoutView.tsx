...
function OrderReview({
  ...
  return (
    <div className="flex h-full flex-col bg-stone-50 font-sans">
      <header className="sticky top-0 z-30 flex items-center justify-center border-b border-black/[0.06] bg-stone-50/95 px-4 py-4 backdrop-blur-xl">
        <button
          type="button"
          onClick={onBack}
          aria-label="Kembali"
          className="absolute left-4 flex h-10 w-10 items-center justify-center rounded-full border border-black/10 bg-white active:scale-95"
        >
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar">
        <section className="bg-stone-50 px-4 pb-2 pt-5">
          <div className="mb-2 flex items-center justify-between border-b border-stone-200 pb-3">
            ...
            <button
              type="button"
              onClick={onBack}
              className="text-[10px] font-label font-bold uppercase tracking-widest text-black underline underline-offset-4"
            >
              + Tambah menu
            </button>
          </div>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <h3 className="truncate font-sans text-base font-bold leading-tight text-stone-900">{product.name}</h3>
                    ...
                    <div className="mt-4 flex items-center justify-between">
                      <span className="font-sans text-sm font-bold text-black">{formatIDR(unitPrice * cartItem.quantity)}</span>
                      <div className="flex items-center gap-3 rounded-full border border-stone-200 bg-stone-50 p-1">
                        ...
                        <button type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); updateQuantity(cartItem.id, 1); }} className="flex h-7 w-7 items-center justify-center rounded-full bg-black text-white"><Plus className="h-3 w-3" /></button>
                      </div>
                    </div>
                  </div>
                </motion.article>
        ...
        <section className="mx-4 mt-4">
          {appliedCoupon ? (
            ...
            <button
              ...
              className="flex w-full items-center justify-between rounded-2xl border border-stone-200 bg-white p-4 shadow-sm transition-all active:scale-[0.98] hover:border-black"
            >
              <div className="flex items-center gap-3">
                <div className="bg-emerald-50 text-black p-2.5 rounded-xl">
        ...
        <section className="mx-4 mb-32 mt-5 overflow-hidden rounded-[26px] border border-black/[0.07] bg-white">
          <div className="flex items-center gap-3 border-b border-stone-100 px-5 py-4">
            ...
            <div>
              ...
              <h3 className="font-sans text-base font-bold">Rincian pembayaran</h3>
            </div>
          </div>
          <div className="space-y-3 px-5 py-4 text-sm">
            ...
            <button type="button" onClick={() => setFeesExpanded((value) => !value)} className="flex w-full items-center justify-between text-left">
              ...
              <span className="text-xs font-bold text-black">{feesExpanded ? 'Sembunyikan' : 'Lihat rincian'}</span>
            </button>
            ...
            <div className="flex items-end justify-between border-t border-dashed border-stone-200 pt-4">
              ...
              <span className="font-sans text-2xl font-bold text-black">{formatIDR(total)}</span>
            </div>
          </div>
        </section>
      </div>
      ...
      <div className="absolute inset-x-0 bottom-0 z-20 border-t border-stone-100 bg-white/95 px-6 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-4 shadow-[0_-16px_30px_rgba(0,0,0,0.06)] backdrop-blur-xl">
        <button
          ...
          className="flex w-full items-center justify-between rounded-2xl bg-black px-5 py-4 text-white shadow-lg shadow-black/10 transition active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-40"
        >
          <span>
            <span className="block text-left text-[9px] font-label uppercase tracking-[0.25em] text-white/65">Lanjut pembayaran</span>
            <span className="font-sans text-lg font-bold">{formatIDR(total)}</span>
          </span>
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between border-b border-stone-100 p-6 shrink-0">
                <h3 className="font-sans text-xl font-bold text-stone-900">Pilih Promo</h3>
                ...
                <div className="flex gap-2 relative">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-stone-200 bg-white py-3.5 pl-11 pr-3 text-sm font-bold uppercase outline-none focus:border-black focus:ring-4 focus:ring-[var(--color-primary)]/10 transition"
                    />
                  </div>
                  <button
                    type="button"
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar">
                ...
                {availablePromos && availablePromos.length > 0 ? (
                  availablePromos.map((promo: any) => {
                    ...
                    return (
                      <div key={promo.id} className="relative overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-sm transition-all hover:border-black hover:shadow-md group">
                        ...
                        <div className="p-5 relative">
                          <div className="flex gap-4 items-start">
                            <div className="w-14 h-14 bg-gradient-to-br from-[var(--color-primary)] to-emerald-900 rounded-2xl flex flex-col items-center justify-center text-white shrink-0 shadow-lg shadow-black/10">
                              {isPercentage ? <Percent className="w-6 h-6" /> : <Tag className="w-6 h-6" />}
                            </div>
                            <div className="flex-1 min-w-0">
                              <h4 className="font-sans text-xl font-black text-stone-900 leading-none mb-1.5 truncate">{discountLabel}</h4>
                              ...
                              <div className="flex flex-wrap gap-1.5 mt-3">
                                ...
                                {(promo.is_auto_apply || promo.is_auto_apply === 1) && (
                                  <span className="text-[9px] font-bold uppercase tracking-wider text-black bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-100">
                                    Auto Apply
                                  </span>
                                )}
                        ...
                        <div className="border-t border-dashed border-stone-200 bg-stone-50 p-4 flex items-center justify-between relative">
                          ...
                          <button onClick={() => handleApplyFromSheet(code)} className="bg-black text-white text-xs font-bold px-6 py-3 rounded-xl shadow-md shadow-black/10 active:scale-95 transition-all hover:brightness-110">
                            Gunakan
                          </button>
                        </div>
                      </div>
...
function PaymentStep({ onBack, onPay, total, isProcessing, slug }: any) {
  ...
  return (
    <div className="flex h-full flex-col bg-stone-50 font-sans">
      <header className="sticky top-0 z-30 border-b border-black/[0.06] bg-stone-50 px-4 pb-5 pt-[calc(1rem+env(safe-area-inset-top))]">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[9px] font-label uppercase tracking-[0.4em] text-black/70">Langkah 2 dari 2</p>
            <h1 className="font-sans text-2xl font-bold tracking-tight">Detail <span className="text-black">Pembayaran</span></h1>
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 space-y-8 overflow-y-auto px-6 pb-36 pt-8 no-scrollbar">
        <div className="rounded-[24px] border border-black/[0.07] bg-white p-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3 rounded-2xl border border-stone-100 bg-stone-50 px-4 py-3.5 focus-within:border-black"><User className="h-4 w-4 shrink-0 text-stone-400" /><input type="text" placeholder="Full Name" value={name} onChange={(event) => setName(event.target.value)} className="w-full bg-transparent text-sm font-medium text-stone-800 outline-none" required /></div>
            <div className="flex items-center gap-3 rounded-2xl border border-stone-100 bg-stone-50 px-4 py-3.5 focus-within:border-black"><Mail className="h-4 w-4 shrink-0 text-stone-400" /><input type="email" placeholder="Email Address" value={email} onChange={(event) => setEmail(event.target.value)} className="w-full bg-transparent text-sm font-medium text-stone-800 outline-none" required /></div>
            <div className="flex items-center gap-3 rounded-2xl border border-stone-100 bg-stone-50 px-4 py-3.5 focus-within:border-black"><Phone className="h-4 w-4 shrink-0 text-stone-400" /><input type="tel" placeholder="Phone Number (e.g., 0812...)" value={phone} onChange={(event) => setPhone(event.target.value.replace(/\D/g, ''))} className="w-full bg-transparent text-sm font-medium text-stone-800 outline-none" required /></div>

            <div className="space-y-3">
              <div className={`flex items-center gap-3 rounded-xl border px-3 py-3 ${serviceType === 'dine_in' ? 'border-black bg-emerald-50' : 'border-stone-200 bg-stone-100 opacity-70'}`}>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setServiceType('dine_in')} disabled={!tableCode} className={`rounded-xl border px-3 py-3 text-xs font-bold transition ${serviceType === 'dine_in' ? 'border-black bg-emerald-50 text-black' : 'border-stone-200 bg-white text-stone-500'} disabled:cursor-not-allowed disabled:opacity-40`}>Makan di Tempat</button>
                  <button type="button" onClick={() => setServiceType('takeaway')} className={`rounded-xl border px-3 py-3 text-xs font-bold transition ${serviceType === 'takeaway' ? 'border-black bg-emerald-50 text-black' : 'border-stone-200 bg-white text-stone-500'}`}>Takeaway</button>
                </div>
        ...
        <div className="rounded-[24px] border border-black/[0.07] bg-white p-5">
          ...
          <div className="mb-4 grid grid-cols-2 gap-2">
            {(['online', 'cashier'] as const).map((type) => (
              <button key={type} type="button" onClick={() => { setOrderType(type); setMethod(type === 'online' ? 'qris' : 'cash'); }} className={`rounded-xl border py-2.5 text-xs font-bold ${orderType === type ? 'border-black bg-emerald-50 text-black' : 'border-stone-200 bg-stone-50'}`}>
          ...
          <div className="space-y-2">
            {visibleMethods.map((payment) => (
              <button key={payment.id} type="button" onClick={() => setMethod(payment.id)} className={`flex w-full items-center justify-between rounded-xl border p-3.5 ${method === payment.id ? 'border-black bg-emerald-50' : 'border-stone-100'}`}>
                <div className="flex items-center gap-3">
                ...
                <div className={`flex h-4 w-4 items-center justify-center rounded-full border-2 ${method === payment.id ? 'border-black' : 'border-stone-300'}`}>
                  {method === payment.id && <div className="h-2 w-2 rounded-full bg-black" />}
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="absolute inset-x-0 bottom-0 z-20 border-t border-stone-100 bg-white/95 px-6 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-4 shadow-[0_-16px_30px_rgba(0,0,0,0.06)] backdrop-blur-xl">
        <div className="mb-3 flex items-end justify-between">
          <div><p className="text-[9px] font-label font-bold uppercase tracking-[0.25em] text-stone-400">Total pembayaran</p><p className="font-sans text-xl font-bold text-stone-900">{formatIDR(total)}</p></div>
        ...
        <button 
          ...
          className="flex w-full items-center justify-center gap-2 rounded-2xl bg-black py-4 font-bold text-white shadow-lg shadow-black/10 disabled:opacity-50 transition active:scale-[0.99]"
        >
...
function QrisStep({ onBack, onFinish, qrUrl, qrString, total, orderCode, expiryTime }: any) {
  ...
  return (
    <div className="flex h-full flex-col bg-stone-50 font-sans">
      <header className="flex items-center gap-3 border-b border-black/[0.06] bg-stone-50 px-4 py-4 text-black">
      ...
      <div className="flex flex-1 flex-col items-center space-y-5 overflow-y-auto bg-stone-50 p-4">
        <div className="relative flex w-full max-w-sm flex-col items-center overflow-hidden rounded-[28px] border border-black/[0.07] bg-white p-6 shadow-sm">
          ...
          <div className={`mb-6 mt-4 rounded-xl border-4 bg-white p-3 flex flex-col items-center justify-center h-auto w-64 ${isExpired ? 'border-red-200 grayscale' : 'border-black'}`}>
            {finalQrSrc ? (
              <>
          ...
          <p className="mb-4 text-2xl font-black text-black">{formatIDR(total)}</p>
      ...
      <div className="z-20 border-t border-black/[0.07] bg-stone-50 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))]">
        <button
          type="button"
          onClick={handleMainButtonClick}
          disabled={isChecking}
          className={`flex w-full items-center justify-center gap-3 rounded-xl py-4 font-bold text-white transition-all ${
            isExpired ? 'bg-stone-400 hover:bg-stone-500' : 'bg-black active:scale-95'
          }`}
        >
          {isChecking ? (
...
export default function CheckoutView({ onBack, onSuccess }: Props) {
  ...
  if (step === 'review' && cartItems.length === 0 && !currentOrderPayload) {
    return (
      <div className="flex min-h-full flex-col items-center justify-center bg-stone-50 px-6 py-12 text-center">
        ...
        <p className="mb-2 text-[9px] font-bold uppercase tracking-[0.28em] text-black">Keranjang kosong</p>
        <h1 className="font-sans text-2xl font-bold text-stone-900">Belum ada pesanan</h1>
        <button type="button" onClick={onBack} className="mt-7 rounded-2xl bg-black px-6 py-3.5 text-sm font-bold text-white shadow-lg active:scale-[0.97]">Jelajahi Menu</button>
      </div>
    );
  }

  return (
    <div className="checkout-view-container h-full bg-stone-50 font-sans">
