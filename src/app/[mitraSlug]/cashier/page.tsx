...
export default function CashierApp() {
  ...
  const [role, setRole] = useState<"cashier" | "owner" | "kitchen" | null>(
    null,
  );
  ...
  const [mitraProfile, setMitraProfile] = useState<{ name: string }>({
    name: "Kasir",
  });
  ...
  const [scanningTransport, setScanningTransport] = useState<
    "usb" | "bluetooth" | null
  >(null);
  const logoFileInputRef = useRef<HTMLInputElement | null>(null);

  const [printerSettings, setPrinterSettings] =
    useState<CashierPrinterSettings>(DEFAULT_PRINTER_SETTINGS);
  ...
  useEffect(() => {
    ...
    let reconnectController: ReturnType<
  ...
  const saveAllPrinterSettings = async () => {
    ...
    try {
      ...
      Toast.fire({
        icon: "error",
        title: "Pengaturan printer gagal disimpan",
      });
    } finally {
  ...
  const handleScanPrinter = async (transport?: "usb" | "bluetooth") => {
    ...
    try {
      ...
      setPrinters((current) => {
        ...
        return Array.from(
          new Map(
            merged.map((printer) => [`${printer.type}:${printer.id}`, printer]),
          ).values(),
        );
      });
      ...
      Toast.fire({
        icon: "error",
        title:
          error instanceof Error ? error.message : "Gagal mendeteksi printer",
      });
    } finally {
  ...
  const saveSelectedPrinter = async () => {
    ...
    Toast.fire({
      icon: "success",
      title: `${selectedPrinter.name} disimpan`,
    });
  };
  ...
  const removeSavedPrinter = (printer: PrinterDevice) => {
    ...
    setPrinters((current) =>
      current.filter(
        (item) => !(item.id === printer.id && item.type === printer.type),
      ),
    );
    ...
    Toast.fire({
      icon: "success",
      title: "Printer dihapus dari daftar tersimpan",
    });
  };
  ...
  const handleLogoUpload = async (
    ...
    if (!allowedTypes.includes(file.type)) {
      Toast.fire({
        icon: "error",
        title: "Logo hanya boleh PNG, JPG, JPEG, atau WEBP",
      });
      return;
    }
    ...
    if (file.size > 2 * 1024 * 1024) {
      Toast.fire({
        icon: "error",
        title: "Ukuran logo maksimal 2 MB",
      });
      return;
    }
    ...
    try {
      ...
      Toast.fire({
        icon: "success",
        title: "Logo berhasil disimpan pada banner mitra",
      });
    } catch (error) {
      ...
      Toast.fire({
        icon: "error",
        title: error instanceof Error ? error.message : "Logo gagal diunggah",
      });
    }
  };
  ...
  const handleTestPrint = async () => {
    ...
    try {
      ...
      Toast.fire({
        icon: "error",
        title:
          error instanceof Error ? error.message : "Printer belum terhubung",
      });
    }
  };
          ...
          if (prev.length > 0 && result.data.length > prev.length) {
            if (audioRef.current) {
              ...
              audioRef.current.play().catch((e) => {
                console.warn(
                  "Audio diblokir: Kasir harus klik layar/tekan tombol minimal 1x setelah refresh.",
                  e,
                );
              });
            }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (staffRole !== "cashier" && staffRole !== "owner") {
          Toast.fire({
            icon: "error",
            title: "Akses Ditolak! QR ini tidak memiliki izin Kasir.",
          });
          return;
        }
        ...
        Toast.fire({
          icon: "success",
          title: `Selamat Bekerja, ${staffName}!`,
        });
      } else {
  ...
  const handlePOSSubmit = async (newOrder: Order) => {
    ...
    try {
      ...
      console.error("[AUTO_PRINT_CUSTOMER_ERROR]", {
        error,
        orderId: newOrder.id,
        orderCode: newOrder.order_code,
        slug,
        activePrinter: PrinterManager.getPrinter(slug),
      });
  ...
  const todayOrders = useMemo(() => {
    ...
    return orders.filter(
      (o) => new Date(o.createdAt || o.created_at || 0).toDateString() === t,
    );
  }, [orders]);
  const totalRevenue = useMemo(
    () =>
      todayOrders.reduce(
        (s, o) => s + (Number(o.totalPrice || o.total_price) || 0),
        0,
      ),
    [todayOrders],
  );
    ...
    () =>
      orders
        ...
        .sort((a, b) => {
          ...
          const dateA = String(a.createdAt || a.created_at || 0).replace(
            " ",
            "T",
          );
          const dateB = String(b.createdAt || b.created_at || 0).replace(
            " ",
            "T",
          );
          return (
            (new Date(dateB).getTime() || 0) - (new Date(dateA).getTime() || 0)
          );
        }),
    [orders, activeTab],
  );
  ...
  if (!isAuthenticated) {
    return (
      <div
        style={{
          minHeight: "100dvh",
          background: "linear-gradient(160deg, #f6f3ee 0%, #e8e2d9 100%)",
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          padding: "16px",
      ...
      >
        ...
        >
          ...
          <p className="text-xs text-stone-500 mt-1.5 text-center px-4 mb-6">
            Arahkan QR Code Karyawan ke kamera atau gunakan Scanner Fisik
          </p>
              ...
              <>
                {!isNative && isScanning ? (
                  ...
                  >
                    ...
                    {isNative ? "Scan QR Native" : "Buka Kamera Web"}
                  </button>
                )}
                ...
                <div className="text-center p-4 bg-emerald-50/50 rounded-xl border border-emerald-100">
                  <p className="text-[11px] font-medium text-emerald-800 leading-relaxed">
                    <strong className="font-bold">
                      Scanner fisik juga aktif.
                    </strong>{" "}
                    <br />
                    Langsung *Tembak* QR Code ke layar.
                  </p>
                </div>
              </>
            )}
      ...
      >
        <div
          style={{
            ...
            display: "flex",
            flexDirection: "column",
            boxShadow: "0 0 40px rgba(28,28,25,0.1)",
        ...
        >
          ...
          >
            ...
            >
              ...
              <span>
                Login:{" "}
                <strong className="text-stone-800">{activeStaffName}</strong>{" "}
                (Owner)
              </span>
            </div>
            <button
  ...
  // @ts-ignore
  const popupTotalBill = cashPaymentPopup
    ? Number(
        cashPaymentPopup.totalAfterDiscount ||
          cashPaymentPopup.total_after_discount ||
          cashPaymentPopup.totalPrice ||
    ...
    >
      <div
        style={{
          ...
          display: "flex",
          flexDirection: "column",
          boxShadow: "0 0 40px rgba(28,28,25,0.12)",
          position: "relative",
          overflow: "hidden",
        }}
      >
            ...
            >
              ...
              >
                <div className="p-5 border-b border-stone-100 flex items-center justify-between bg-stone-50">
                  <div>
                    <h3 className="text-lg font-black text-stone-800 tracking-tight leading-none">
                      Terima Tunai
                    </h3>
                    <p className="text-[11px] font-bold uppercase tracking-widest text-stone-400 mt-1">
                      Order #{cashPaymentPopup.id}
                    </p>
                  </div>
                  <button
                ...
                <div className="p-6 space-y-6 overflow-y-auto">
                  <div className="text-center p-6 rounded-2xl bg-amber-50 border border-amber-100">
                    ...
                    <p className="text-4xl font-black text-amber-600 font-display">
                      {formatPrice(popupTotalBill)}
                    </p>
                  </div>

                  <div>
                    <label className="text-[11px] font-bold uppercase tracking-widest text-stone-500 mb-2 block">
                      Uang Diterima (Rp)
                    </label>
                    <input
                      type="text"
                      ...
                      value={
                        receivedAmount
                          ? formatPrice(
                              Number(receivedAmount.replace(/\D/g, "")),
                            )
                              .replace("Rp", "")
                              .trim()
                          : ""
                      ...
                      onChange={(e) =>
                        setReceivedAmount(e.target.value.replace(/\D/g, ""))
                      }
                    ...
                    <div className="flex gap-2 mt-3 overflow-x-auto pb-1 custom-scrollbar">
                      <button
                        onClick={() =>
                          setReceivedAmount(String(popupTotalBill))
                        }
                        className="shrink-0 px-4 py-2 rounded-lg bg-stone-100 text-stone-700 text-xs font-bold border border-stone-200 hover:bg-stone-200"
                      >
                        Uang Pas
                      </button>
                      <button
                  ...
                  <div className="flex justify-between items-center p-4 rounded-xl border border-stone-100 bg-stone-50">
                    ...
                    <span
                      className={`text-lg font-black ${popupChange < 0 ? "text-red-500" : "text-emerald-600"}`}
                    >
                      {popupChange < 0
                        ? "Uang Kurang"
                        : formatPrice(popupChange)}
                    </span>
                  </div>
                </div>
        ...
        <AnimatePresence>
          {isPOSMode && (
            <CashierPOS
              onClose={() => setIsPOSMode(false)}
              onSubmitOrder={handlePOSSubmit}
            />
          )}
        </AnimatePresence>
              ...
              >
                <header className="flex items-center justify-between border-b border-stone-200 bg-gradient-to-r from-[#0E5C37] to-emerald-600 px-5 py-4 text-white sm:px-6 sm:py-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-black">Pengaturan Printer</h3>
                  ...
                  {[
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setPrinterSettingsTab(tab.id as PrinterSettingsTab)
                        }
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="mb-3 flex items-center justify-between">
                          <div>
                            ...
                            <p className="mt-1 text-xs text-stone-400">
                              Deteksi printer thermal melalui USB atau
                              Bluetooth, lalu pilih perangkat yang digunakan.
                            </p>
                          </div>
                          ...
                          <div>
                            ...
                            {printers.length === 0 && !isScanningPrinter && (
                              <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50 px-5 py-8 text-center">
                                ...
                                <p className="mt-1 text-xs text-stone-400">
                                  Sambungkan kabel USB atau aktifkan Bluetooth,
                                  lalu tekan tombol deteksi.
                                </p>
                              </div>
                            )}
                      ...
                      >
                        <NumberStepper
                          ...
                          max={5}
                          onChange={(value) =>
                            updatePrinterSetting("copies", value)
                          }
                        />
                      </SettingGroup>
                    </div>
                  ...
                  {printerSettingsTab === "receipt" && (
                    <div className="space-y-5">
                      <SettingToggle
                        ...
                        onChange={(checked) =>
                          updatePrinterSetting("showLogo", checked)
                        }
                      />
                              ...
                              {[
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    updatePrinterSetting(
                                      "logoSize",
                                      option.id as ReceiptLogoSize,
                                    )
                                  }
                                  className={`min-h-11 rounded-xl border-2 text-xs font-black ${
                                    printerSettings.logoSize === option.id
                                      ? "border-[#0E5C37] bg-emerald-50 text-[#0E5C37]"
                  ...
                  {printerSettingsTab === "content" && (
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      {[
                        ...
                        ["showDiscount", "Diskon"],
                        ["showTax", "Pajak"],
                        ["showServiceCharge", "Biaya layanan"],
          ...
          {notification && (
            <motion.div
              ...
              style={{
                ...
                padding: "12px 16px",
                borderRadius: "12px",
                boxShadow: "0 8px 24px rgba(14,92,55,0.3)",
                display: "flex",
                alignItems: "center",
                gap: "8px",
                fontSize: "13px",
          ...
          {undoAction && (
            <motion.div
              ...
              style={{
                ...
                padding: "12px 16px",
                borderRadius: "12px",
                boxShadow: "0 8px 32px rgba(28,28,25,0.25)",
                display: "flex",
                justifyContent: "space-between",
                alignItems: "center",
              }}
        ...
        <header
          style={{
            ...
            justifyContent: "space-between",
            alignItems: "center",
            boxShadow: "0 2px 12px rgba(28,28,25,0.05)",
            flexShrink: 0,
            zIndex: 30,
          }}
        >
            ...
            <div style={{ textAlign: "center", padding: "60px 20px" }}>
              ...
              >
                {activeTab === "pending"
                  ...
                  : activeTab === "preparing"
                    ? "Daftar pesanan yang sedang diracik."
                    : activeTab === "ready"
                      ? "Pesanan siap disajikan ke pelanggan."
        ...
        {role === "cashier" && (
          <button
            ...
            style={{
              ...
              border: "none",
              cursor: "pointer",
              boxShadow: "0 8px 24px rgba(14,92,55,0.4)",
            }}
          >
            <Plus size={24} />
          </button>
  ...
  return (
    <span
      className={`relative h-7 w-12 shrink-0 rounded-full transition ${
        checked ? "bg-[#0E5C37]" : "bg-stone-300"
      }`}
    >
      <span
...
function ReceiptPreview({
  ...
  const previewWidth =
    settings.paperWidth === "80mm" ? "max-w-[360px]" : "max-w-[280px]";

  return (
    <div className="rounded-2xl bg-stone-200 p-4 sm:p-6">
      <div
        className={`mx-auto bg-white px-4 py-5 font-mono text-[11px] leading-relaxed text-stone-800 shadow-xl ${previewWidth}`}
      >
        {settings.showLogo && (
        ...
        {settings.showCashier && (
          <PreviewRow label="Kasir" value={cashierName || "Kasir"} />
        )}

        {settings.showCustomer && (
          <PreviewRow label="Pelanggan" value="Pelanggan Umum" />
        )}
        ...
        <div className="space-y-2">
          <div>
            ...
            {settings.showAddons && (
              <p className="pl-3 text-stone-500">+ Extra shot</p>
            )}
          </div>
        ...
        {settings.showSubtotal && (
          <PreviewRow label="Subtotal" value="48.000" />
        )}

        {settings.showDiscount && <PreviewRow label="Diskon" value="-3.000" />}
        ...
        {settings.showPaymentMethod && (
          <PreviewRow label="Pembayaran" value="Tunai" />
        )}

        {settings.showCashReceived && (
          <PreviewRow label="Diterima" value="50.000" />
        )}

        {settings.showChange && <PreviewRow label="Kembalian" value="5.000" />}
        ...
        {settings.thankYouText && (
          <p className="mt-4 text-center font-bold">{settings.thankYouText}</p>
        )}

        <div
          style={{
            height: `${settings.feedLines * 5}px`,
          }}
        />
      </div>
    </div>
