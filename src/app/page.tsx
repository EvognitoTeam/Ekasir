...
const payments = ['QRIS', 'GoPay', 'OVO', 'DANA', 'ShopeePay', 'Transfer', 'Cash'];
...
const attendanceAddon = {
  ...
  price: 'Rp1.500.000',
  period: '/tahun',
  description: 'Sistem absensi karyawan terintegrasi dengan dashboard dan outlet EKASIR.',
  features: [
...
export default function EvokasirLandingPage() {
  ...
  const whatsappUrl = useMemo(() => {
    const message = encodeURIComponent(`Halo tim EKASIR,\n\nSaya tertarik untuk menjadwalkan demo EKASIR.\n\nProduk yang diminati: ${selectedPlan}\n\nMohon informasikan jadwal demo, mekanisme fee platform flat 1,4% untuk seluruh transaksi, dan detail layanan EKASIR.\n\nTerima kasih.`);
    return `https://wa.me/${whatsappNumber}?text=${message}`;
  }, [selectedPlan]);

  const emailUrl = useMemo(() => {
    const subject = encodeURIComponent(`Permintaan Demo EKASIR - ${selectedPlan}`);
    const body = encodeURIComponent(`Halo tim EKASIR,\n\nSaya tertarik untuk menjadwalkan demo EKASIR.\n\nProduk yang diminati: ${selectedPlan}\n\nMohon informasikan jadwal demo, mekanisme fee platform flat 1,4% untuk seluruh transaksi, dan detail layanan EKASIR.\n\nTerima kasih.`);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[var(--color-surface)] text-[var(--color-on-surface)]">
      ...
      <nav className="relative z-20 w-full px-6 py-6">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="ambient-shadow flex h-11 w-11 items-center justify-center rounded-2xl bg-[var(--color-primary)] text-white">
              <ScanLine size={22} />
            </div>
            <div>
              <h1 className="text-2xl font-black tracking-tight text-[var(--color-primary)]">EKASIR</h1>
      ...
      <section id="paket" className="relative z-10 mx-auto max-w-7xl px-4 py-24">
        <div className="mx-auto mb-16 max-w-3xl text-center">
          <p className="mb-4 text-xs font-black uppercase tracking-[0.3em] text-[var(--color-primary)]">Harga EKASIR</p>
          <h2 className="text-4xl font-black leading-tight md:text-5xl">Mulai Gratis dengan Fee Platform Flat 1,4%</h2>
          <p className="mt-5 text-lg text-[var(--color-on-surface-variant)]">Tidak ada biaya langganan software bulanan. Setiap transaksi berhasil yang diproses atau dicatat melalui EKASIR dikenakan fee platform tetap sebesar 1,4%, tanpa membedakan metode pembayaran.</p>
        </div>
        ...
        <div className="grid grid-cols-1 items-stretch gap-7 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="relative flex flex-col overflow-hidden rounded-[2rem] border border-[var(--color-primary)] bg-[var(--color-primary)]/5 p-8 shadow-2xl backdrop-blur-xl md:p-10">
            ...
            <div className="relative z-10 flex h-full flex-col">
              ...
              <h3 className="mt-7 text-3xl font-black">EKASIR Gratis</h3>
              <p className="mt-3 text-sm leading-relaxed text-[var(--color-on-surface-variant)]">Gunakan seluruh fitur utama EKASIR untuk mengelola transaksi, pesanan, pelanggan, dan operasional bisnis F&amp;B.</p>
              ...
              <button type="button" onClick={() => openDemoModal('EKASIR Gratis dengan Fee Platform Flat 1,4%')} className="ambient-shadow mt-auto flex w-full items-center justify-center gap-3 rounded-2xl bg-[var(--color-primary)] px-6 py-4 font-black text-white transition-all hover:scale-[1.02]">Jadwalkan Demo <CalendarDays size={18} /></button>
            </div>
          </div>
          ...
          <div className="relative overflow-hidden rounded-[2rem] border border-[var(--color-outline-variant)]/20 bg-[var(--color-surface-container)]/40 p-8 backdrop-blur-xl md:p-10">
            ...
            <div className="relative z-10">
              ...
              <h3 className="text-3xl font-black leading-tight">Satu Tarif untuk Seluruh Transaksi EKASIR</h3>
              <p className="mt-4 leading-relaxed text-[var(--color-on-surface-variant)]">Seluruh transaksi berhasil yang diproses atau dicatat melalui EKASIR dikenakan fee platform sebesar 1,4%, baik pembayaran dilakukan menggunakan tunai, QRIS, e-wallet, transfer, maupun metode pembayaran lainnya.</p>
              ...
              <div className="mt-7 rounded-2xl border border-[var(--color-outline-variant)]/20 bg-[var(--color-surface)]/60 p-5">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-sm leading-relaxed text-[var(--color-on-surface-variant)]">Fee platform dihitung dari nilai transaksi berhasil yang tercatat melalui EKASIR. Biaya MDR, settlement, atau biaya penyedia pembayaran dapat berlaku secara terpisah sesuai metode pembayaran yang digunakan.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mx-auto mt-8 max-w-5xl">
          <div className="rounded-2xl border border-[var(--color-outline-variant)]/20 bg-[var(--color-surface-container)]/40 p-5">
            <p className="text-center text-sm leading-relaxed text-[var(--color-on-surface-variant)]">Fee platform flat 1,4% dikenakan pada seluruh transaksi berhasil yang diproses atau dicatat melalui EKASIR, termasuk transaksi tunai dan non-tunai. Biaya MDR, settlement, atau biaya penyedia pembayaran dapat berlaku secara terpisah.</p>
          </div>
        </div>
      </section>
        ...
        <div className="relative overflow-hidden rounded-[2.5rem] border border-[var(--color-outline-variant)]/20 bg-[var(--color-primary)] p-10 text-white md:p-14">
          ...
          <div className="relative z-10 grid grid-cols-1 items-center gap-10 lg:grid-cols-2">
            <div>
              ...
              <p className="mt-5 leading-relaxed text-white/80">Konsultasikan kebutuhan sistem kasir, QR menu, dan digitalisasi bisnis F&amp;B Anda bersama tim EKASIR.</p>
      ...
      <footer className="relative z-10 border-t border-[var(--color-outline-variant)]/20 px-4 py-10">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 md:flex-row">
          <div>
            <h3 className="text-xl font-black text-[var(--color-primary)]">EKASIR</h3>
      ...
      {isDemoModalOpen && (
        <div role="dialog" aria-modal="true" aria-labelledby="demo-modal-title" onClick={closeDemoModal} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm">
          <div onClick={(event) => event.stopPropagation()} className="relative w-full max-w-lg rounded-[2rem] border border-[var(--color-outline-variant)]/20 bg-[var(--color-surface)] p-7 shadow-2xl md:p-9">
            ...
            <h2 id="demo-modal-title" className="pr-10 text-3xl font-black leading-tight">Hubungi Tim EKASIR</h2>
            <p className="mt-4 leading-relaxed text-[var(--color-on-surface-variant)]">Pilih metode komunikasi yang paling nyaman. Tim EKASIR akan membantu menjadwalkan demo dan menjelaskan paket yang sesuai.</p>
